...
def get_skolereg(skolereg_aar: str | int = 'latest') -> pd.DataFrame:
    ...
    if skolereg_aar == 'latest':
        ...
        assert len(str(skolereg_aar)) == 4, 'året du oppga for skoleregfila er på feil format. vi ønsker YYYY.'  # assert year format
        skolereg_filename = [file for file in skolereg_files if file[1:5] == str(skolereg_aar)]
...
def get_omk_kat(omk_kat_aar: str | int = 'latest') -> pd.DataFrame:
    ...
    omk_files = [file for file in omk_files if file.split('_')[0] == 'testomkkat'] # this needs to be updated for use outside test-scope
    ...
    if omk_kat_aar == 'latest':
        ...
        omk_filename = [file for file in omk_files if file.split('_')[1][:4] == str(omk_kat_aar)]
        omk_filename = omk_filename[0] #retrieving the newest secondary catalogue from the year asked for
...
def evaluate_skolereg_merge(innfil: pd.DataFrame,
    ...
    print(f"{len(kobler_orgnr)}/{len(innfil)} koblet mot skolereg på '{orgnr_col_innfil}'")
    ...
    print(f"Av de som ikke koblet på '{orgnr_col_innfil}' var det {len(kobler_orgnrbed)}/{len(koblet_ikke_orgnr)} som koblet på '{orgnrbed_col_innfil}'")
    
    koblet_ikke =  koblet_ikke_orgnr.loc[~koblet_ikke_orgnr[orgnrbed_col_innfil].isin(skolereg['orgnrbed'])]
    print(f"Totalt var det {len(koblet_ikke)}/{len(innfil)} som ikke koblet på hverken '{orgnr_col_innfil}' eller '{orgnrbed_col_innfil}' \n")
    
    return kobler_orgnr, kobler_orgnrbed, koblet_ikke


def evaluate_omk_kat_merge(koblet_ikke: pd.DataFrame, 
    ...
    print(f"Med utgangspunkt i de {len(koblet_ikke)} som ikke koblet, sjekker vi kobling mot sekundærkatalog på nøkkel 'fskolenr'")
    print(f"Finner {len(koblet_omk)}/{len(koblet_ikke)} på 'fskolenr' i omkodingskatalogen")
    
    koblet_ikke_omk = koblet_ikke.loc[~koblet_ikke['fskolenr'].isin(omk_kat['fskolenr'])]
    # isolating the information in the secondary catalogue relevant for the people not found in the 'skolereg' so far
    omk_kat_eval = omk_kat.loc[omk_kat['fskolenr'].isin(koblet_omk['fskolenr'].unique())]
    
    # trying to see if the 'orgnr' in the secondary catalogue exists in 'skolereg'
    print(f"De {len(koblet_omk)} som kobler mot sekundærkatalog er representert av {len(omk_kat_eval)} 'fskolenr'")
    omk_orgnr_skolereg = omk_kat_eval.loc[omk_kat_eval['orgnr_omk'].isin(skolereg['orgnr'])]
    omk_orgnrbed_skolereg = omk_kat_eval.loc[omk_kat_eval['orgnrbed_omk'].isin(skolereg['orgnrbed'])]
    print(f"Av disse {len(omk_kat_eval)} får vi {len(omk_orgnr_skolereg)} treff mot skolereg på 'orgnr' og {len(omk_orgnrbed_skolereg)} treff mot 'orgnrbed' \n")
    

    # finding those who can merge with 'skolereg' after updating their 'orgnr' from the secondary catalogue
    kobl_omk_skolereg_orgnr = koblet_omk.loc[koblet_omk['fskolenr'].isin(omk_orgnr_skolereg['fskolenr'])]
    kobl_omk_skolereg_orgnrbed = koblet_omk.loc[koblet_omk['fskolenr'].isin(omk_orgnrbed_skolereg['fskolenr'])]
    
    # finding those who were found in the secondary catalogue, but did not 
    kobl_omk_not_skolereg = koblet_omk.loc[(~koblet_omk['fskolenr'].isin(omk_orgnr_skolereg['fskolenr']) & \
                                  (~koblet_omk['fskolenr'].isin(omk_orgnrbed_skolereg['fskolenr'])))]
    
    # verifying that all are accounted for
    assert len(kobl_omk_not_skolereg) + len(kobl_omk_skolereg_orgnr) + len(kobl_omk_skolereg_orgnrbed) +  len(koblet_ikke_omk) == len(koblet_ikke)
    return kobl_omk_not_skolereg, kobl_omk_skolereg_orgnr, kobl_omk_skolereg_orgnrbed, koblet_ikke_omk


def merge_skolereg(innfil: pd.DataFrame,
    ...
    if not isinstance(kobler_orgnr, pd.DataFrame) or not isinstance(kobler_orgnrbed, pd.DataFrame):
        kobler_orgnr, kobler_orgnrbed, koblet_ikke = evaluate_skolereg_merge(innfil, skolereg, orgnr_col_innfil, orgnrbed_col_innfil)
...
def merge_omk(koblet_ikke: pd.DataFrame,
    ...
    assert 'fskolenr' in list(koblet_ikke.columns), 'fskolenr ligger ikke som kolonne på DataFramen som skal kobles mot sekundærkatalogen'
    ...
    # optional separation of what merges on different 'orgnr' column
    if not isinstance(kobl_omk_not_skolereg, pd.DataFrame) or not isinstance(kobl_omk_skolereg_orgnr, pd.DataFrame) or not isinstance(kobl_omk_skolereg_orgnrbed, pd.DataFrame):
        kobl_omk_not_skolereg, kobl_omk_skolereg_orgnr, kobl_omk_skolereg_orgnrbed, koblet_ikke_omk = evaluate_omk_kat_merge(koblet_ikke, 
                                                                                                                             omk_kat, 
                                                                                                                             skolereg,
                                                                                                                             orgnr_col_innfil,
                                                                                                                             orgnrbed_col_innfil
    ...
    kobl_omk_skolereg_orgnr = kobl_omk_skolereg_orgnr.merge(omk_kat[['fskolenr', 'orgnr_omk']], how='left', on='fskolenr')
    kobl_omk_skolereg_orgnrbed = kobl_omk_skolereg_orgnrbed.merge(omk_kat[['fskolenr', 'orgnrbed_omk']], how='left', on='fskolenr')
    
    
    # merging with those found in skolereg
    merged_omk_skolereg_orgnr = kobl_omk_skolereg_orgnr.merge(skolereg[skolereg_keep_cols], how='left', left_on='orgnr_omk', right_on='orgnr')
    merged_omk_skolereg_orgnrbed = kobl_omk_skolereg_orgnrbed.merge(skolereg[skolereg_keep_cols], how='left', left_on='orgnrbed_omk', right_on='orgnrbed')
    
    # merging with secondary catalogue for those who didn't merge with skolereg
    merged_omk_not_skolereg = kobl_omk_not_skolereg.merge(omk_kat, how='left', on='fskolenr')
    if include_kobler_ikke:
        return pd.concat([merged_omk_skolereg_orgnr, merged_omk_skolereg_orgnrbed, merged_omk_not_skolereg, koblet_ikke_omk])
    return pd.concat([merged_omk_skolereg_orgnr, merged_omk_skolereg_orgnrbed, merged_omk_not_skolereg])


def iterate_edit(to_omk_nodup: pd.DataFrame,
                 skolereg: pd.DataFrame
                ) -> pd.DataFrame:
    print(f"Nå vil vi iterete over de {len(to_omk_nodup)} skolene som ikke ble funnet i skoleregisteret. Finn riktig orgnr og fyll inn.")
    orgnr_list = []
    ...
    for i, row in to_omk_nodup.iterrows():
        ...
        orgnr_omk = input(f"Hva er riktig orgnr for denne skolen?")
        if orgnr_omk not in skolereg['orgnr'].to_list() and orgnr_omk not in skolereg['orgnrbed'].to_list():
            print("Orgnummeret du oppga ble ikke funnet i skoleregisteret og editeringen vil derfor ikke lagres i sekundærkatalogen.")
        ...
        while in_skolereg.lower() not in yeslist:
            display(pd.DataFrame(row).T)
            orgnr_omk = input("Skriv inn riktig orgnr for skolen, for stopp, skriv 'stopp': ")
            if orgnr_omk.lower() == 'stopp':
                print("\n Da går vi videre")
                in_skolereg = 'y'
            if orgnr_omk in skolereg['orgnr'].to_list() or orgnr_omk in skolereg['orgnrbed'].to_list():
                in_skolereg = 'y'
...
def write_omk_kat(to_write: pd.DataFrame):
    ...
    if omk_kat.equals(to_write):
        print('Sekundærkatalogen du forsøker å skrive til fil er identisk med nyeste versjon på disk. Skriver ikke duplikat')
    else:
...
def manual_omk(to_manuel_omk: pd.DataFrame,
               skolereg: pd.DataFrame,
               omk_kat: pd.DataFrame,
               orgnr_col_innfil: str = 'orgnr',
               orgnrbed_col_innfil: str = 'orgnrbed'
    ...
    if skolenavn not in to_manuel_omk.columns:
        skolenavn = input("Hva heter kolonnen som inneholder skolenavn (feks. 'snavn') på datafilen? ")
    skolenr = 'fskolenr'
    if skolenr not in to_manuel_omk.columns:
        skolenr = input("Hva heter kolonnen som inneholder skolenr (feks. 'fskolenr') på datafilen? ")
    cols_to_omk = [orgnr_col_innfil, orgnrbed_col_innfil, skolenavn, skolenr]
    
    to_omk = to_manuel_omk[cols_to_omk].copy()
    to_omk_nodup = to_omk.drop_duplicates(subset=[orgnr_col_innfil, orgnrbed_col_innfil, skolenr])
    
    from_omk_nodup = iterate_edit(to_omk_nodup, skolereg)
    mask_kobler = (from_omk_nodup['orgnr_omk'].isin(skolereg['orgnr'])) | (from_omk_nodup['orgnr_omk'].isin(skolereg['orgnrbed']))
    to_write = from_omk_nodup.loc[mask_kobler]
    #to_write.drop(columns=[orgnr_col_innfil, orgnrbed_col_innfil], inplace=True)
    
    rename_dict = {skolenavn: 'fskolenavn', 
                   skolenr: 'fskolenr'}
    ...
    if len(to_write)>0:
        ...
        print(f"Totalt {len(to_write)}/{len(to_omk_nodup)} fikk oppdatert orgnr med ett som kobler mot skolereg. \n \
...
def orgnrkontroll(innfil: pd.DataFrame,
    ...
    merged_w_skolereg = merge_skolereg(innfil, 
                                       skolereg_keep_cols, 
                                       orgnr_col_innfil, 
                                       orgnrbed_col_innfil,
                                       kobler_orgnr,
    ...
    print("Evaluerer kobling mot skolereg gjennom sekundærkatalogen på nytt etter manuel koding av orgnr")
    omk_kat = get_omk_kat('latest')
    kobl_omk_not_skolereg, kobl_omk_skolereg_orgnr, kobl_omk_skolereg_orgnrbed, koblet_ikke_omk = evaluate_omk_kat_merge(koblet_ikke,
                                                                                                                         omk_kat,
                                                                                                                         skolereg
                                                                                                                        )
    merged_w_omk = merge_omk(koblet_ikke,
                             skolereg_keep_cols,
                             orgnr_col_innfil,
                             orgnrbed_col_innfil,
...
if __name__ == '__main__':
    # print(get_omk_kat())
    merged = orgnrkontroll(data, orgnr_col_innfil = 'orgnr_inn', orgnrbed_col_innfil = 'forgnr', skolereg_keep_cols='nace1_sn07')
