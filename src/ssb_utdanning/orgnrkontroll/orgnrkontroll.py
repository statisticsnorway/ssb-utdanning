...
def evaluate_skolereg_merge(
    ...
    print(
        f"{len(kobler_orgnr)}/{len(innfil)} rader fra innfila koblet mot skolereg med '{orgnr_col_innfil}'"
    )
    ...
    print(
        f"Av radene som ikke koblet på '{orgnr_col_innfil}' var det {len(kobler_orgnrbed)}/{len(koblet_ikke_orgnr)} rader som koblet på '{orgnrbed_col_innfil}'"
    )
    ...
    print(
        f"Totalt var det {len(koblet_ikke)}/{len(innfil)} rader fra innfila som ikke koblet på hverken '{orgnr_col_innfil}' eller '{orgnrbed_col_innfil}' \n"
    )
    return kobler_orgnr, kobler_orgnrbed, koblet_ikke


def evaluate_vigo_merge(
    koblet_ikke_skolereg: pd.DataFrame,
    vigo: pd.DataFrame,
    fskolenr_innfil: str = "fskolenr",
) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    print(
        f"Evaluerer kobling mot vigo skoleverk for de {len(koblet_ikke_skolereg)} radene på innfila som ikke koblet mot skolereg på orgnr/orgnrbed"
    )
    ...
    print(
        f"{len(kobler_fskolenr)}/{len(koblet_ikke_skolereg)} koblet på 'fskolenr' mot vigo skoleverk \n"
    )
...
def merge_vigo(
    ...
    merged_fskolenr = kobler_fskolenr.merge(
        vigo, how="left", left_on=fskolenr_innfil, right_on="SKOLENR"
    )
