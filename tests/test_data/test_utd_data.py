...
class TestUtdData(unittest.TestCase):
    ...
    def test_init(self):
        ...
        data = UtdData(self.data, self.path_to_file, glob_pattern="ignored_glob_pattern*.parquet")
        self.assertIsInstance(data, UtdData)
        self.assertIsInstance(data.data, pd.DataFrame)
        self.assertEqual(self.path_to_file, data.path)
        self.assertTrue(self.data.equals(data.data))
        ...
        data = UtdData(glob_pattern=str(self.path)+"/datatest*.parquet", exclude_keywords=["drop", "keyword"])
        self.assertIsInstance(data, UtdData)
        self.assertIsInstance(data.data, pd.DataFrame)
        ...
        data = UtdData(glob_pattern=str(self.path)+"/data_*.parquet", exclude_keywords=["drop", "keyword"])
    ...
    def test_correct_check_path(self):
        ...
        self.assertIsNone(data._correct_check_path("/invalid_path/nonexistentfile_p2024-10_v1.parquet"))
    ...
    def test_get_latest_version_path(self):
        ...
        self.assertTrue(int(str(path_bumped)[-9]) == int(str(self.path_to_file)[-9])+1)
