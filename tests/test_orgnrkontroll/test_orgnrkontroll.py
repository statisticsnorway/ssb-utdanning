...
from create_mock_orgnr_files import create_mock_skolereg, create_mock_vigo, create_mock_data
import unittest
...
@mock.patch('ssb_utdanning.orgnrkontroll.orgnrkontroll.VIGO_PATH', new=os.getcwd() + '/mock_data/')
@mock.patch('ssb_utdanning.orgnrkontroll.orgnrkontroll.SKOLEREG_PATH', new=os.getcwd() + '/mock_data/')
class Test_orgnrkontroll(unittest.TestCase):
    ...
    def test_get_skolereg(self):
        ...
        self.assertTrue(str(result.path).split('/')[-1] == 'skolereg_p2023-10_v1.parquet')
        del result
        result=orgnrkontroll_module.get_skolereg(year=2022,sub_category='vgskoler')
        self.assertTrue(str(result.path).split('/')[-1] == 'skolereg_vgskoler_p2022-10_v1.parquet')
    ...
    def test_get_vigo_skole(self):
        ...
        self.assertTrue(str(result.path).split('/')[-1], 'vigo_skole_testfil_slett_p2023_v1.parquet')
    ...
    def test_orgnrkontroll_func(self):
        ...
        with self.assertRaises(ValueError):
            result = orgnrkontroll_module.orgnrkontroll_func(data=data, 
                                                             year=year,
        ...
        with self.assertRaises(TypeError):
            result = orgnrkontroll_module.orgnrkontroll_func(data=data, 
                                                             year=year,
                                                             vigo_keep_cols = 'not set or list'
                                                            )
